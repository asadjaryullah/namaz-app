      ...
      const { error } = await supabase.auth.signInWithOtp({
        ...
        options: {
          ...
          data: {
            full_name: fullName,
            phone: phone,
            gender: gender // <--- WIRD JETZT MITGESENDET
          }
        },
      });
              ...
              <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div 
                    ...
                    className={`cursor-pointer rounded-lg border p-2 text-center text-sm font-bold flex flex-col items-center transition-all
                      ${gender === 'male' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-500 border-slate-200'}`}
                  >
                    <span>🧔🏻‍♂️ Bruder</span>
                  </div>
                  <div 
                    ...
                    className={`cursor-pointer rounded-lg border p-2 text-center text-sm font-bold flex flex-col items-center transition-all
                      ${gender === 'female' ? 'bg-pink-600 text-white border-pink-600' : 'bg-white text-slate-500 border-slate-200'}`}
                  >
                    <span>🧕🏻 Schwester</span>
                  </div>
                </div>
