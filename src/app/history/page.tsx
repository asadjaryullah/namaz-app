...
export default function HistoryPage() {
  ...
  // TABS: Wir lesen den Tab aus der URL, sonst Standard 'zikr'
  ...
  // Funktionen für Zikr & Kalender (identisch wie vorher)
  ...
  const handleReset = (e: React.MouseEvent, key: string) => {
    e.stopPropagation(); if(!confirm("Reset?")) return;
    const newData = { ...zikrData, [key]: 0 }; setZikrData(newData); saveToDb(newData);
  };
  ...
  const getRingStyle = (count: number) => {
    ...
    let c = '#cbd5e1'; 
    if (count >= 5) c = '#16a34a'; else if (count >= 3) c = '#3b82f6'; else if (count > 0) c = '#f97316'; 
          ...
          {activeTab === 'zikr' && (
            <div className="space-y-4 animate-in fade-in slide-in-from-left-4 duration-300">
              {ZIKR_LIST.map((item) => {
                ...
                return (
                  <Card key={item.key} onClick={() => !isDone && handleZikrClick(item.key, item.target)} className={`relative overflow-hidden transition-all duration-200 border-0 shadow-md ${isDone ? 'bg-emerald-500 cursor-default' : `cursor-pointer active:scale-95 ${item.theme.bg} ${item.theme.text}`}`}>
                    ...
                    {count > 0 && !isDone && (<div className="absolute top-3 left-3 z-10"><button onClick={(e) => handleReset(e, item.key)} className="p-1.5 bg-white/60 rounded-full text-slate-400 hover:text-red-500 hover:bg-white transition-all shadow-sm"><RotateCcw size={14} /></button></div>)}
                    <div className="p-5 flex items-start justify-between gap-4">
                      <div className="shrink-0 flex flex-col items-center justify-center min-w-[3.5rem] pt-2"><span className={`text-3xl font-black ${isDone ? 'text-white' : item.theme.text}`}>{count}</span><span className={`text-[9px] font-bold uppercase ${isDone ? 'text-emerald-100' : 'opacity-60'}`}>{isDone ? 'FERTIG' : `von ${item.target}`}</span></div>
                      <div className="flex-1 flex flex-col items-end text-right"><p className={`text-xs font-bold uppercase mb-2 tracking-widest ${isDone ? 'text-emerald-100' : 'opacity-60'}`}>{item.title}</p><p className={`text-xl font-bold leading-loose font-arabic ${isDone ? 'text-white' : 'text-slate-800'}`} style={{ fontFamily: 'var(--font-amiri)', direction: 'rtl', lineHeight: '1.8' }}>{item.arabic}</p><p className={`text-xs mt-3 italic leading-relaxed text-right w-full ${isDone ? 'text-emerald-100' : 'text-slate-500'}`}>{item.translation}</p></div>
                    </div>
                  </Card>
                )
              })}
          ...
          {activeTab === 'events' && (
             <div className="space-y-4 animate-in fade-in duration-300">
               <Card className="p-5 border-l-4 border-l-orange-500 shadow-sm bg-white">
                 ...
                 <p className="text-sm text-slate-500 mb-4">Hier siehst du alle kommenden Termine.</p>
               ...
               <div className="space-y-3">
                 {events.length === 0 ? <p className="text-center text-slate-400 py-4">Keine Termine geplant.</p> : events.map(e => (
                   <div key={e.id} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex gap-4">
                      <div className="bg-slate-100 p-3 rounded-xl text-center min-w-[4rem]"><span className="block text-xs font-bold text-slate-400 uppercase">{new Date(e.event_date).toLocaleDateString('de-DE', {month: 'short'})}</span><span className="block text-2xl font-black text-slate-800">{new Date(e.event_date).getDate()}</span></div>
                      <div><h3 className="font-bold text-slate-900">{e.title}</h3><p className="text-xs text-slate-500 mt-1 uppercase font-bold tracking-wider">{new Date(e.event_date).toLocaleTimeString('de-DE', {hour:'2-digit', minute:'2-digit'})} UHR</p>{e.description && <p className="text-sm text-slate-600 mt-2">{e.description}</p>}</div>
                   </div>
                 ))}
               </div>
             </div>
