...
export default function TodayCard(p: Props) {
  ...
  useEffect(() => {
    if (!p.nextPrayer) { setMinsLeft(null); return; }
    const compute = () => {
      const diff = toMinutes(p.nextPrayer!.time) - getBerlinMinutes();
  ...
  const isClose = minsLeft !== null && minsLeft <= 30;
  const countdown = minsLeft === null
    ? '—'
    : minsLeft >= 60
      ? `${Math.floor(minsLeft / 60)}h ${minsLeft % 60}m`
      : `${minsLeft} Min`;
  const icon = PRAYER_ICONS[p.nextPrayer.name] || '🕌';
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--app-text2)' }}>{p.nextPrayer.time} Uhr</p>
          </div>
        ...
        {/* Alle Gebetszeiten des Tages */}
        {p.allPrayers.length > 0 && (
          <div className="mt-4 pt-3 flex gap-1" style={{ borderTop: '1px solid var(--app-border)' }}>
            {p.allPrayers.map((pr) => {
              const isNow = pr.id === p.nextPrayer!.id;
              return (
                <div key={pr.id}
                  className="flex-1 min-w-0 flex flex-col items-center gap-0.5 py-1.5 rounded-lg"
                  style={isNow
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-wide truncate max-w-full px-0.5"
                    style={{ color: isNow ? 'var(--app-gold)' : 'var(--app-text3)' }}>
                    {pr.name}
                  </span>
                  <span className="font-mono-app text-[11px] font-bold tabular-nums"
                    style={{ color: isNow ? 'var(--app-gold)' : 'var(--app-text2)' }}>
                    {pr.time}
                  </span>
                </div>
              );
          ...
          <div className="mt-3 pt-3 flex flex-col gap-2" style={{ borderTop: '1px solid var(--app-border)' }}>
            <button
              ...
              {p.togglingCommit
                ? '...'
                : p.isCommitted
                  ? `Ich komme ✓${p.commitmentCount > 0 ? ` · ${p.commitmentCount}` : ''}`
                  : `Ich komme${p.commitmentCount > 0 ? ` · ${p.commitmentCount} zugesagt` : ''}`}
            </button>
